...
test('callback handler should log a sanitized error and return 401 when token exchange fails', async () => {
    ...
    try {
        ...
        assert.strictEqual(loggedErrors[0].payload.name, 'HTTPError');
        assert.strictEqual(loggedErrors[0].payload.message, 'entra-token-exchange-failed');
        assert.strictEqual(loggedErrors[0].payload.code, 'ERR_NON_2XX_3XX_RESPONSE');
        assert.strictEqual(loggedErrors[0].payload.statusCode, 401);
