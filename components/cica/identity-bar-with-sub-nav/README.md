...
| title.html | string | No | HTML title (takes precedence over text) - **⚠️ WARNING: Only pass sanitized or trusted HTML. Unsanitized user input poses an XSS risk** |
...
When using the `title.html` parameter, ensure that any dynamic content is properly sanitized:

- **Use `title.text` for user-provided data** - This parameter automatically escapes HTML
- **Only use `title.html` with trusted, sanitized content** - The `| safe` filter bypasses Nunjucks' auto-escaping
- **Validate and sanitize** any user input before passing it to `title.html`

```nunjucks
{# SAFE - Using title.text for user data #}
{{ cicaIdentityBarWithSubNav({
    title: {
        text: "CRN: " + userProvidedCaseNumber
    }
}) }}
...
{# UNSAFE - Passing unsanitized user input to title.html #}
{{ cicaIdentityBarWithSubNav({
    title: {
        html: '<strong>CRN: ' + userInput + '</strong>'  {# XSS RISK! #}
    }
}) }}
```
