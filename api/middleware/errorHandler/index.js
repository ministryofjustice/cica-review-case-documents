...
function getCustomOpenApiErrorDetail(fullError) {
    ...
    const fieldErrorMessages = validationMessages[fieldName];
    if (!fieldErrorMessages) {
        return message;
    }

    const schemaValidationProperties = Object.keys(fieldErrorMessages);
    const schemaValidationPropertyWithCustomError = schemaValidationProperties.find(
        (validationProperty) => errorCode.includes(validationProperty)
    );

    return fieldErrorMessages[schemaValidationPropertyWithCustomError] || message;
}
